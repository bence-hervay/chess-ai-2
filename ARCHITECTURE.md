...
| `src/games/chess.rs` | Standard chess via cozy-chess; threefold + fifty-move draws via reset-on-irreversible hash history; 780 raw features / 4168 perspective actions; UCI move-notation helpers. |
| `src/search.rs` | Exact memoized WDL solver + reachable-state enumeration (research/test oracle); production alpha–beta with iterative deepening, transposition table, deterministic node budgets, per-iteration best-move ladder, and `Evaluator`-supplied leaf values + policy move ordering. |
| `src/data.rs` | SHSD teacher records: target provenance (§19.9), positions as replayable action paths, deterministic hash-sampled trajectory positions with visit weights, parallel shallow/deep/counterfactual labelling, retrograde-oracle join, summary aggregation. |
...
| `src/bin/lab.rs` | Typed CLI: `lab evaluate` (arena / oracle probe / match probe), `lab solve`, `lab train`, `lab selfplay` (Expert Iteration), `lab teacher` (Stockfish diagnostic), `lab sweep` (CPU-slot scheduler). |
| `src/bin/uci.rs` | UCI chess engine over the shared search + compiled inference; node, depth, movetime, and clock budgets. |

## Not yet present (deferred to their phases)

Quiescence and speculative pruning (gated behind Phase 7+ horizon
measurements), asynchronous UCI stop, PGN tooling beyond fastchess
output.
